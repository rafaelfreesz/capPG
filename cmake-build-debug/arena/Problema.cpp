...
void Problema::go() {
 ...
 bL1() ;
    ...
    for(int i=0;i<this->n;i++){
        this->corredorSolucao[i]=melhorCorredor[i];
    }
...
Problema::Problema(int qtdSalas) {
    ...
    this->desconstruidas=new int[this->n];
}
...
void Problema::rec(float tamF){
    ...
    if(tam<this->n) {
        ...
        for (int i = 0; i < tam; i++) {
            int j = rand() % this->n;
            this->desconstruidas[i] = this->corredorSolucao[j];
        ...
        for (int i = 0; i < tam; i++) {
            this->corredorSolucao[this->n] = this->desconstruidas[i];
            this->n++;
        }
